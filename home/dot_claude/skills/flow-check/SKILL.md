---
name: flow-check
description: Finds what this session decided, left open, or built that the tracker doesn't hold yet, and captures it before you clear or compact. Use before ending or clearing a long session, or when resuming one.
disable-model-invocation: true
---

# Flow check

The conversation is the input. The tracker, git and the PR host are checked only for items the conversation raised. Capturing an item puts it on the tracker; deciding what it needs (spec, ADR, nothing) happens later, in the skill ask-matt routes that issue to.
...
5. Finish with `All captured: yes`. Then route each thread's issue by the main flow in ask-matt, and make the continue, clear or compact call from its `PHASE-BOUNDARIES.md`.
