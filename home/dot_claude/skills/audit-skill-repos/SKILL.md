...
This skill is **chezmoi-managed** (source: `home/dot_claude/skills/audit-skill-repos/`). Edit via the `/edit-governance` flow (chezmoi source, then `chezmoi apply`) — a target-only edit gets reverted on the next apply.
...
- **Moving a skill to a plugin sweeps one path, not two.** A plugin serves Claude Code alone, so keep the repo in `skills.yaml` scoped to the other agents via `agents:` and sweep only `.claude/skills/<name>` — the symlink is what makes Claude register a marketplace-less `<name>@skills-dir` plugin. Codex, Cursor and GitHub Copilot read `~/.agents/skills` directly; deleting that payload uninstalls the skill from them on every apply.
